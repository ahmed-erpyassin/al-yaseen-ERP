<template>
    <div class="container pe-5 ps-5">
        ...
        <div class="d-flex align-items-center justify-content-end">
            <router-link :to="{ name: 'admin.accounting.funders.create' }" class="btn btn-lg btn-main me-3">{{
                $t('buttons.create')
                }}</router-link>
        ...
        <div class="row">
            <div class="col-12">
                <h3 class="mb-5">{{ $t('label.funders') }}</h3>
            </div>
        </div>
        <div class="d-flex align-items-center actions mb-3">
            <div class='search me-2 mb-3'>
                <i class="bi bi-search me-2"></i>
                <span class="text-main">{{ $t('label.search_funder') }}</span>
            </div>
        ...
        return {
            table: {
                fields: [
                    ...
                    {
                        name: this.$t('label.statement_en'),
                        status: true
                    },
                    {
                        name: this.$t('label.country'),
                        status: true
                    },
                    {
                        name: this.$t('label.responsible'),
                        status: true
                    },
                    {
                        name: this.$t('label.work_phone'),
                        status: true
                    },
                    {
                        name: this.$t('label.fax'),
                        status: true
                    }
                ]
