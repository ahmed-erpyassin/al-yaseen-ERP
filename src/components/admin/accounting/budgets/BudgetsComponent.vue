<template>
    <div class="container pe-5 ps-5">
        ...
        <div class="d-flex align-items-center justify-content-end">
            <router-link :to="{ name: 'admin.accounting.budgets.create' }" class="btn btn-lg btn-main me-3">{{
                $t('buttons.create')
                }}</router-link>
        ...
        <div class="row">
            <div class="col-12">
                <h3 class="mb-5">{{ $t('label.budgets') }}</h3>
            </div>
        </div>
        <div class="d-flex align-items-center actions mb-3">
            <div class='search me-2 mb-3'>
                <i class="bi bi-search me-2"></i>
                <span class="text-main">{{ $t('label.search_budget') }}</span>
            </div>
    ...
    data() {
        ...
        return {
            table: {
                fields: [
                    {
                        name: this.$t('label.budget_number'),
                        status: true
                    },
                    {
                    ...
                    {
                        name: this.$t('label.start_date'),
                        status: true
                    },
                    {
                        name: this.$t('label.end_date'),
                        status: true
                    },
                    {
