<template>
    ...
    <div class="container pe-5 ps-5">
        <h1><i class="bi bi-cart"></i> {{ $t('label.incoming_orders') }}</h1>

        <div class="d-flex align-items-center justify-content-end mb-3">
            <!-- Create Button -->
            <router-link :to="{ name: 'admin.orders.create' }" class="btn btn-lg btn-main me-3">
                {{ $t('buttons.create') }}
            </router-link>
...
export default {
    ...
    data() {
        return {
            ...
            table: {
                fields: [
                    { name: 'رقم الطلب', key: 'order_number', status: true },
                    { name: 'اسم العميل', key: 'customer_name', status: true },
                    { name: 'المشغل المرخّص', key: 'licensed_operator', status: true },
                    { name: 'المبلغ', key: 'amount', status: true },
                    { name: 'العملة', key: 'currency', status: true },
                    { name: 'تاريخ الطلب', key: 'date', status: true },
    ...
    computed: {
        ...
        filteredItems() {
            ...
            return this.items.filter(item =>
                (item.order_number?.toString().toLowerCase().includes(query)) ||
    ...
    methods: {
        async fetchItems() {
            ...
            try {
                ...
                const res = await axios.get(`${this.baseUrl}/incoming-orders`, {
                    headers: { Authorization: `Bearer ${token}` }
                });
                this.items = (res.data.data || []).map((o, i) => ({
                    id: o.id || i + 1,
                    order_number: o.order_number || `ORD-${i + 1}`,
                    customer_name: o.customer?.company_name || '-',
                    licensed_operator: o.licensed_operator || '-',
        ...
        viewItem(item) { this.$router.push({ name: 'admin.incoming-orders.show', params: { id: item.id } }); },
        editItem(item) { this.$router.push({ name: 'admin.incoming-orders.edit', params: { id: item.id } }); },
            ...
            reader.onload = e => {
                ...
                jsonData.forEach(item => {
                    this.items.push({
                        ...
                        order_number: item.order_number || `ORD-${this.items.length + 1}`,
        ...
        exportExcel() {
            ...
            XLSX.utils.book_append_sheet(wb, ws, "IncomingOrders");
            const wbout = XLSX.write(wb, { bookType: "xlsx", type: "array" });
            saveAs(new Blob([wbout], { type: "application/octet-stream" }), "IncomingOrders.xlsx");
        },
        exportPDF() {
            const doc = new jsPDF();
...
<style scoped>
.header th { background-color: #F4FFF0 !important; }
.btn-main {
...
.action-icon.bi-eye { color: #0d6efd; }
.action-icon.bi-pencil { color: #ffc107; }
.action-icon:hover { transform: scale(1.2); opacity: 0.8; }
</style>
