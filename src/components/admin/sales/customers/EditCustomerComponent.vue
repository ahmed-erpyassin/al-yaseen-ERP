<template>
  <div class="container pe-5 ps-5">
    <h1><i class="bi bi-image"></i> {{ isEdit ? t('label.edit_customer') : t('label.add_customer') }}</h1>
        ...
        <div class="col-12 mb-4">
          ...
          <div class="d-flex align-items-center">
            ...
            <div class="form-check">
              <input class="form-check-input" type="radio" name="client_type" id="business" value="business"
                v-model="form.client_type" />
          ...
          <div class="item mb-4">
            <div class="mb-3 position-relative">
              ...
              <input v-if="field.type !== 'textarea' && field.type !== 'file' && field.type !== 'select'"
                :type="field.type" :id="field.id" class="form-control rounded-1" v-model="form[field.model]"
                :maxlength="field.maxlength" :class="{ 'is-invalid': errors[field.model] }" />
              ...
              <select v-if="field.type === 'select'" :id="field.id" class="form-control" v-model="form[field.model]">
                <option v-for="option in field.options" :key="option.value" :value="option.value">{{ option.label }}
                </option>
              </select>

              <input v-if="field.type === 'file'" type="file" :ref="field.ref" multiple style="display:none"
...
const props = defineProps({
  customer: { type: Object, default: null } // إذا تم تمرير عميل، سيكون التعديل
});
...
const saveForm = () => {
  ...
  // هنا يمكن إضافة منطق الحفظ عبر API
  Swal.fire({
    icon: 'success',
    ...
    successMsg.value = isEdit.value
      ? t('messages.customer_updated_successfully')
      : t('messages.customer_created_successfully');

    // بعد الحفظ، العودة لصفحة العملاء
...
const cancelForm = () => {
  Swal.fire({
    ...
    if (result.isConfirmed) router.push({ name: 'admin.customers' });
  });
};
