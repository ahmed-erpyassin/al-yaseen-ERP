<template>
    <div class="container pe-5 ps-5">
        ...
        <h1><i class="bi bi-arrow-counterclockwise"></i> {{ $t('label.sales_return_invoice') }}</h1>

        <!-- أزرار إنشاء واستيراد/تصدير -->
        <div class="d-flex align-items-center justify-content-end mb-3">
            <router-link :to="{ name: 'admin.new-sales-return-invoice' }" class="btn btn-lg btn-main me-3">
                {{ $t('buttons.create') }}
            </router-link>
                ...
                <ul class="dropdown-menu align-center rounded-0 p-2" style="width: 250px;">
                    <li v-for="(field, index) in table.fields" :key="field.key">
                        <div class="form-check">
                            <input class="form-check-input" type="checkbox" v-model="field.status" :id="'col-' + index" />
        ...
        <div class="table-responsive">
            <table class="table table-bordered text-center align-middle">
                ...
                <tbody class="table-body">
                    ...
                    <tr v-else v-for="invoice in paginatedItems" :key="invoice.id">
                        <td v-for="field in visibleFields" :key="field.key">{{ invoice[field.key] || '-' }}</td>
                        <td class="text-center">
                            <i class="bi bi-eye action-icon me-2" :title="$t('buttons.check')"
                                @click="viewItem(invoice)"></i>
                            <i class="bi bi-pencil action-icon" :title="$t('buttons.edit')"
                                @click="editItem(invoice)"></i>
                            <i class="bi bi-trash action-icon" :title="$t('buttons.delete')"
                                @click="deleteItem(invoice)"></i>
                        </td>
                    </tr>
                </tbody>
            </table>
...
export default {
    name: 'SalesReturnInvoicesComponent',
    ...
    methods: {
        async fetchItems() {
            this.isLoading = true;
            try {
                const response = await axios.get('/api/v1/sales-management/return-invoices/list-all');
        ...
        viewItem(item) {
            this.$router.push({ name: 'admin.view-sales-return', params: { id: item.id } });
        },
        editItem(item) {
            const routeExists = this.$router.hasRoute('admin.sales-return.edit');
            if (routeExists) {
                this.$router.push({ name: 'admin.sales-return.edit', params: { id: item.id } });
            } else {
        ...
        deleteItem(item) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    try {
                        await axios.delete(`/api/v1/sales-management/return-invoices/delete-return-invoice/${item.id}`);
            ...
            reader.onload = e => {
                ...
                jsonData.forEach(item => {
                    this.items.push({
                        ...
                        invoice_no: item.invoice_no || `RET-${this.items.length + 1}`,
        ...
        exportExcel() {
            ...
            saveAs(new Blob([wbout], { type: 'application/octet-stream' }), 'SalesReturn.xlsx');
        },
        exportPDF() {
            const doc = new jsPDF();
...
.action-icon.bi-trash { color: #dc3545; }
.action-icon:hover {
    transform: scale(1.2);
    opacity: 0.8;
}
