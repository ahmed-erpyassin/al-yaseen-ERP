<template>
  <div class="container pe-5 ps-5">
    <h1><i class="bi bi-building"></i> {{ t('buttons.edit_branch') }}</h1>
...
// Sample: Replace with actual API fetch if needed
const branchesDB = [
  { id: 1, name: 'فرع مسقط', manager: 'أحمد ياسين', currency: 'OMR', city: 'مسقط', country: 'عمان', status: 'active' },
  { id: 2, name: 'فرع دبي', manager: 'محمد علي', currency: 'AED', city: 'دبي', country: 'الإمارات', status: 'active' }
]
...
onMounted(() => {
  ...
  const existingBranch = branchesDB.find(b => b.id === id)
  if (existingBranch) {
    Object.assign(branch, existingBranch)
  } else {
    Swal.fire({
      icon: 'error',
      title: t('messages.not_found'),
      text: t('messages.branch_not_found')
    }).then(() => router.push({ name: 'admin.branches' }))
  }
})
