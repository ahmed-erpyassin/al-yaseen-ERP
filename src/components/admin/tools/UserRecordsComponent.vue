<template>
    <div class="container pe-5 ps-5">
        ...
        <div class="d-flex align-items-center justify-content-end">
            <router-link :to="{ name: 'admin.tools.users.create' }" class="btn btn-lg btn-main me-3">{{ $t('buttons.create')
            }}</router-link>
        ...
        <div class="row">
            <div class="col-12">
                <h3 class="mb-5">{{ $t('label.user_records') }}</h3>
            </div>
        </div>
        <div class="d-flex align-items-center actions mb-3">
            <div class='search me-2 mb-3'>
                <i class="bi bi-search me-2"></i>
                <span class="text-main">{{ $t('label.search_user') }}</span>
            </div>
        ...
        <div class="d-flex align-items-center justify-content-end">
            <div class="pages">
                ...
                <p class="text-main mb-0">{{ $t('label.user_report') }}</p>
                <p class="text-main mb-0">{{ $t('label.duplicate_user') }}</p>
            </div>
        </div>
    ...
    data() {
        ...
        return {
            table: {
                fields: [
                    {
                        name: this.$t('label.number'),
                        status: true
                    },
                    {
                        name: this.$t('label.username'),
                        status: true
                    },
                    {
                    ...
                    {
                        name: this.$t('label.role_type'),
                        status: true
                    },
                    {
                        name: this.$t('label.last_login_date'),
                        status: true
                    },
                    {
                        name: this.$t('label.active_inactive'),
                        status: true
                    }
                ]
