<template>
    <div class="container pe-5 ps-5">
        ...
        <div class="row">
            <div class="col-12">
                <h3 class="mb-5">{{ $t('label.attendance_sheet') }}</h3>
            </div>
        </div>
        <div class="d-flex align-items-center actions mb-3">
            <div class='search me-2 mb-3'>
                <i class="bi bi-search me-2"></i>
                <span class="text-main">{{ $t('label.search_employee') }}</span>
            </div>
...
export default {
    ...
    data() {
        ...
        return {
            table: {
                fields: [
                    { name: this.$t('label.employee_number'), status: true },
                    { name: this.$t('label.name'), status: true },
                    { name: this.$t('label.department'), status: true },
                    { name: this.$t('label.month_year'), status: true },
                    { name: this.$t('label.date'), status: true },
                    { name: this.$t('label.day'), status: true },
                    { name: this.$t('label.check_out_time'), status: true },
                    { name: this.$t('label.working_hours'), status: true },
                    { name: this.$t('label.overtime_hours'), status: true },
                    { name: this.$t('label.leave_type'), status: false },
                    { name: this.$t('label.basic_salary'), status: false },
                    { name: this.$t('label.other_overtime_hours'), status: false },
                    { name: this.$t('label.deductions'), status: false }
                ]
