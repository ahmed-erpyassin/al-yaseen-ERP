...
export const customer = {
  ...
  actions: {
    ...
    updateCustomer: function (context, payload) {
      ...
      return new Promise((resolve, reject) => {
        axios
          .put(`customers/${payload.id}`, apiPayload, {
            headers: {
              Authorization: "Bearer " + context.rootState.auth.authToken,
              "Content-Type": "application/json",
            },
    ...
    getCustomerFinancialMovements: function (context, customerId) {
      ...
      return new Promise((resolve, reject) => {
        axios
          .get(`customers/${customerId}/financial-movements`, {
            headers: {
              Authorization: "Bearer " + context.rootState.auth.authToken,
            },
          })
          ...
          .catch((err) => {
            context.commit(
              ...
              err.response?.data?.message ||
                "Failed to fetch financial movements"
            );
            reject(err);
          })
          .finally(() => {
    ...
    getCustomerStoreMovements: function (context, customerId) {
      ...
      return new Promise((resolve, reject) => {
        axios
          .get(`customers/${customerId}/store-movements`, {
            headers: {
              Authorization: "Bearer " + context.rootState.auth.authToken,
            },
          })
          ...
          .catch((err) => {
            context.commit(
              "setError",
              err.response?.data?.message || "Failed to fetch store movements"
            );
            reject(err);
          })
          .finally(() => {
  ...
  mutations: {
    ...
    updateCustomer: function (state, updatedCustomer) {
      ...
      if (
        ...
        state.currentCustomer.id === updatedCustomer.id
      ) {
    ...
    removeCustomers: function (state, customerIds) {
      ...
      if (
        state.currentCustomer &&
        customerIds.includes(state.currentCustomer.id)
      ) {
